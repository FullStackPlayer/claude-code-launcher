...
export async function checkClaudeCodeInstalled(): Promise<boolean> {
  try {
    ...
    if (exitCode === 0) {
      ...
      // 尝试自动安装
      Logger.warning('未检测到全局安装的 @anthropic-ai/claude-code，正在尝试自动安装...');
      try {
        const installProc = Bun.spawn(['npm', 'install', '-g', '@anthropic-ai/claude-code'], {
          stdout: 'inherit',
          stderr: 'inherit'
        });
...
export function isExecutable(): boolean {
  if (process.execPath.endsWith('ccl') || process.execPath.endsWith('ccl.exe')) return true;
  return false;
}
...
//   console.log(currentDir, process.cwd())
  ...
  try {
    ...
    Logger.error(`配置文件格式错误: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  }
}
...
function validateConfig(config: AppConfig): void {
  ...
  if (config.default_provider && !config.providers[config.default_provider]) {
    throw new Error(`default_provider "${config.default_provider}" 在 providers 中不存在`);
  }
}
...
    const proc = Bun.spawn([claudeCmd], {
    ...
    Logger.error(`启动 Claude Code 失败: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  }
}